...
int Boss::m_NrOfInstances{};

Boss::Boss(const Point2f& pos) :
	Enemy("Images/Boss.png", pos, 6, Vector2f{ 200.f,0.f }, 3000, 3, 3, 100.f, 10000)
...
void Boss::HandleLevelCollision(const Level& level)
{
...
void Boss::DoFullSpawnAnimation()
...
void Boss::DrawTextures(const Rectf& srcRect) const
{
	glPushMatrix();
	FlipTexture();
...
void Boss::SetSpriteFrame(Rectf& srcRect) const
{
	switch (m_ActionState)
	{
	case ActionState::walking:
...
void Boss::FollowAvatar(const Rectf& avatarShape)
{
...
Rectf Boss::GetWeaponRect() const
{
